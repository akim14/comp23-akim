...
After printing the banner text when not requesting the story file version : say "[line break]You open your eyes and see nothing but darkness. You try to remember what happened, why you're here, who you are, but thinking alone makes your head throb. Dread and discomfort slowly creeps into your body. You shiver. For some reason, you feel the need to explore. You know you can't just stay here. You need to investigate."
...
[The Dark Cell]
The Dark Cell is a room. "The air feels oddly heavy. The dark is unsettling. A bed is at the corner. A small window sheds some light so you can navigate around."
...
Instead of waking the rat:
	 say "The rat squeals and opens its mouth, dropping a card. It scurries away in a mad dash to some obscure place.";
	  move the card to the Bathroom;
	  remove the rat from play;
...
The clipboard is in the Equipment Area. The initial appearance of the clipboard is "On the ground there is a fallen clipboard with some ripped pages." The description of the clipboard is "Many of the pages are ripped out. You can make out some sentences on same pages: [line break]'--exhibiting extraordinary d--' [line break]'--comprehension skills' [line break]'--void outcome.' [line break] Your fingers graze something on the back of the clipboard."

Understand "flip [something] over" and "turn [something] over" as flipping. Flipping is an action applying to one thing.
Instead of flipping the clipboard:
	say "You turn the clipboard over and see a message carved into the wooden surface: [line break]'Wrong. Wrong. All wrongwrongwrong.'";
...
The safelight is in the Red Room. The safelight can be switched on. The initial appearance of the safelight is "Hanging on the wall is a safelight."

[Check status of safelight]	 
lightOn is a number variable. lightOn is 0.
Instead of switching on the safelight:
	if (lightOn is 0):
		say "You turn on the safelight. It emits a dim glow and shines down on a countertop covered in photos. They're half-developed and you can only catch a glimpse of some figures in the photos: all appear to be in the presence of some white-coated individual.";
		now the safelight is switched on;
		increment lightOn;
		increase the score by 3;
	otherwise:
		say "The safelight is already on.".

[The mouse in the Red Room]
The mouse is an animal in the Red Room. The initial appearance of the mouse is "Out of the corner of your eye, you see a white mouse nibbling at something near the wall." The description of the mouse is "It has something in its paws...your feline friend hasn't noticed it yet. Sylvester's too busy grooming his fur. Perhaps if you gestured at the mouse..."

deadmau5 is a number variable. deadmau5 is 0. [check if mouse is dead]
...
The cages is in the Lab. The initial appearance of the cages is "A series of cages line the walls, all open." The description is "Looking into some cages, you see tufts of different fur inside. Were they from different animals?"
After examining the cages:
	increase the score by 3.

The wooden maze is in the Lab. The initial appearance of the wooden maze is "There is a small, hand-crafted, wooden maze on a table." The description of the wooden maze is "It's just the right size for a small animal. It doesn't look too hard to solve. "

Understand "beat [something]" and "solve [something]" as solving. Solving is an action applying to one thing.
...
The drawing is in the Experimental Room. The initial appearance of the drawing is "One drawing in the room is not of an object or person and appears to be marked with something." The description of the drawing is "The drawing is actually a map. It looks to be a map of the building you are in. Tally marks are drawn under the map in pencil, along with the name 'Sebastian Weaver'. 'That's the map of this facility,' the Voice explains. 'As you can see, it's not very big. It's rather obscure. Which is what they wanted.' The Voice falls silent rather curtly.
...
photoSeen is a number variable. photoSeen is 0.
...
Instead of talking the Stranger:
	say "The man does not seem to hear and continues to stare silently at the wall. 'He won't respond,' the Voice says quietly. 'It's a little too late, I'm afraid.' You fall silent and look at the man uneasily.".
...
graffitiSeen is a number variable. graffitiSeen is 0.
...
After examining the graffiti:
	if (graffitiSeen is 0):
		increment graffitiSeen;
		increase the score by 4.
...
Instead of smashing the window with the table leg:
	if (weaponGet is 1):
		...
		remove the table leg from play;
...
[line break]Ah...but of course... Yes. My sliver of hope throughout all these months. The resistor. I feel the urge to watch him again...to observe his steady march through the facility...it only took about two months, even with my intervention, before he experienced amnesiac episodes and forgot the next day what he had done, but he still walked every day...like an automaton.
